...
if [ ! -f "$CONF" ]; then
    ...
    echo ""
    echo "  URL:      https://hourglass.local:8080"
    echo "            (HTTPS is automatic — a local CA is generated and"
    echo "            trusted on first start; falls back to http:// if"
    echo "            the CA could not be installed)"
    echo "  Username: admin"
